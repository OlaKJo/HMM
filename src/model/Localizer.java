...
public class Localizer implements EstimatorInterface {
	...
	/**
	 * Class which uses HMM and forward filtering to estimate the position of a
	 * Robot object in a world of size rows x cols.
	 * 
	 * @param rows
	 * @param cols
	 * @param head
	 ...
	 * set the sizes of the transition matrix and the collection of diagonal
	 * matrices in bigO
	 */
	...
	/**
	 * Implementation of Forward filtering HMM using ft =
	 * norm_factor*O*T_transpose*ftminus1 ft = newAlpha ftminus1 = realAlpha O =
	 * realO T_transpose = realT
	 * 
	 * @param localAlpha
	 * @param sX
	 * @param sY
	...
	/**
	 * Calculate all the emission probibilities, the diagonal matrices, and save the
	 * values as rows in bigO
	 */
	...
	/**
	 * calculates the emission probability for all positions given the input sensory
	 * position returns as an array as follows: {{0,0}N, {0,0}E, {0,0}S, {0,0}W,
	 * {0,1}N, ... , {rows-1, cols-1}W}
	 * 
	 * @param senseX
	 * @param senseY
	 * @return
					...
					for (int xCol = 0; xCol < rows; xCol++) {
						for (int yCol = 0; yCol < cols; yCol++) {
							for (int dirCol = 0; dirCol < 4; dirCol++) {
								...
								T[i][j] = getTransProb(xRow, yRow, Heading.values()[dirRow], xCol, yCol,
										Heading.values()[dirCol]);
								j++;
							}
						}
					}
	...
	/**
	 * return the probability of moving from (x_tminus1, y_tminus1) in
	 * heading_tminus1, and ending up in (x_t, y_t) in heading_t
	 * 
	 * @param x_tminus1
	 * @param y_tminus1
	 ...
	 * @return
	 */
	private double getTransProb(int x_tminus1, int y_tminus1, Heading heading_tminus1, int x_t, int y_t,
			Heading heading_t) {
	...
	/**
	 * returns the positions that are reachable in the next step from a given
	 * position
	 * 
	 * @param x
	 * @param y
	...
	/**
	 * Returns a list of the closest AND farthest neighbors of (x,y) using the
